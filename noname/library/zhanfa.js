  ...
  zf_tipo: {
    ...
    skill: {
      ...
      init(player, skill) {
        ...
        next.set("num", num);
        next.setContent(async (event, trigger, player2) => {
          const { num: num2 } = event;
          await player2.gainMaxHp(num2);
  ...
  zf_wendingtizhi: {
    ...
    skill: {
      ...
      init(player, skill) {
        ...
        next.set("player", player);
        next.setContent(async (event, trigger, player2) => {
          const num = 7 - player2.maxHp;
          await player2[num > 0 ? "gainMaxHp" : "loseMaxHp"](Math.abs(num));
          await player2.recover(Math.abs(num));
        });
      },
      ...
      async content(event, trigger, player) {
        trigger.set("chooseToDiscard", async (event2, player2, target) => {
          ...
          if (result?.bool && result.cards?.length) {
            await player2.showCards(result.cards, `${get.translation(player2)}因<span class = bluetext>虚焰</span>修改过的【火攻】展示的牌`);
          }
          return result;
        });
      }
  ...
  zf_zhudao: {
    ...
    skill: {
      ...
      async cost(event, trigger, player) {
        ...
        event.result = await player.chooseCard(get.prompt2(event.skill), select, (card, player2) => player2.canRecast(card), "he").set("ai", (card) => 6 - get.value(card)).forResult();
      },
      async content(event, trigger, player) {
        const { cards } = event;
