...
const doudizhu = {
  ...
  start: [
    ...
    async (event, trigger, player) => {
      ...
      game.broadcast((map2) => {
        for (const i in map2) {
          lib.playerOL[i].identity = map2[i];
      ...
      if (_status.mode === "online") {
        ...
        next.num = (player2) => {
          ...
          if (lib.character[player2.name1] && get.infoHp(lib.character[player2.name1][2]) > 3) {
            num++;
          }
          if (lib.character[player2.name2] && get.infoHp(lib.character[player2.name2][2]) > 3) {
            num++;
          }
          return num;
        };
      } else if (_status.mode === "binglin") {
        next.num = (player2) => player2 === game.zhu ? 5 : 4;
      }
  ...
  game: {
    ...
    chooseCharacterZhidou() {
      ...
      next.setContent(async (event, trigger, player) => {
        ...
        for (const current2 of game.players) {
          const id = current2.playerid;
          current2._group = groups.randomRemove(1)[0];
          characterMap[id] = charactersByGroup[current2._group].randomGets(4);
          current2.storage.doudizhu_cardPile = get.cards(20).sort((a, b) => {
            if (a.name !== b.name) {
        ...
        while (!biddingFinished) {
          ...
          const result2 = await current.chooseControl({
            controls: [...controls],
            ai: () => controls.randomGet()
          }).forResult();
          current.classList.remove("glow_phase");
          current._control = result2.control;
          current.chat(result2.control);
          if (result2.control === "三倍") {
            game.bonusNum = 3;
            game.zhu = current;
            break;
          }
          if (result2.control !== "不叫地主") {
            controls.splice(1, controls.indexOf(result2.control));
            tempDizhu = current;
            if (result2.control === "二倍") {
              game.bonusNum = 2;
            }
          }
        ...
        for (const current2 of game.players) {
          current2.identity = current2 === game.zhu ? "zhu" : "fan";
          current2.showIdentity();
        }
        ...
        for (const current2 of game.players) {
          if (current2 !== game.me) {
            if (current2 === game.zhu) {
              const characters = characterMap[current2.playerid].randomGets(2);
              current2.init(characters[0], characters[1]);
            } else {
              current2.init(characterMap[current2.playerid].randomGet());
            }
          }
          current2.markSkill("doudizhu_cardPile");
        }
        game.zhu.hp = 4;
        game.zhu.maxHp = 4;
        game.zhu.update();
        for (const current2 of game.players) {
          _status.characterlist.remove(current2.name1);
          _status.characterlist.remove(current2.name2);
        }
    ...
    chooseCharacterBinglin() {
      ...
      next.setContent(async (event, trigger, player) => {
        ...
        _status.characterlist = characterList.slice();
        for (const current2 of game.players) {
          characterMap[current2.playerid] = characterList.randomRemove(4);
        }
        const controls = ["不叫地主", "一倍", "两倍", "三倍"];
        ...
        if (game.me.identity === "fan") {
          const friend = game.findPlayer((current2) => current2 !== game.me && current2.identity === "fan");
        ...
        }).set("list", characterMap[game.me.playerid]).forResult();
        ...
        for (const current2 of game.players) {
          ...
          if (current2 === game.zhu) {
            current2.addSkill(game.zhuSkill);
          } else {
            current2.addSkill("binglin_neihong");
          }
        }
    ...
    chooseCharacterHuanle() {
      ...
      next.setContent(async (event, trigger, player) => {
        ...
        while (!biddingFinished) {
          ...
          const result2 = await choiceEvent.forResult();
          current.chat(result2.control);
          if (result2.control === "叫地主" || current === start.next) {
            game.zhu = result2.control === "叫地主" ? current : current.next;
            biddingFinished = true;
          } else {
            current = current.next;
      ...
      next.setContent([
        ...
        async (event, trigger, player) => {
          ...
          const addSetting = (dialog2) => {
            ...
            const identityList2 = ["random", "zhu", "fan"];
            for (const identity of identityList2) {
              ...
              td.addEventListener(lib.config.touchscreen ? "touchend" : "click", (e) => {
                ...
                _status.event.parent.swapnodialog = (dialog3, list2) => {
                  const buttons = ui.create.div(".buttons");
                  const node = dialog3.buttons[0].parentNode;
                  dialog3.buttons = ui.create.buttons(list2, "characterx", buttons);
            ...
            for (let seat = 2; seat <= game.players.length; seat++) {
              ...
              td.addEventListener(lib.config.touchscreen ? "touchend" : "click", (e) => {
                ...
                target.classList.add("bluebg");
                for (const current2 of game.players) {
                  if (get.distance(current2, game.me, "absolute") === target.link) {
                    game.swapSeat(game.zhu, current2, false);
                    return;
                  }
                }
              });
          ...
          const removeSetting = () => {
            ...
            const list2 = Array.from(dialog2.querySelectorAll(".add-setting"));
            while (list2.length) {
              list2.shift().remove();
            }
            ui.update();
      ...
      next.setContent(async (event, trigger, player) => {
        ...
        game.broadcastAll((map2) => {
          for (const id in map2) {
            lib.playerOL[id]._characterChoice = map2[id];
          }
        }, map);
        const choices = [];
        for (const current of game.players) {
        ...
        const result = await game.me.chooseButtonOL(choices, (current, choiceResult) => {
        ...
        game.broadcast(
          (result2, zhu) => {
            for (const id in result2) {
              if (!lib.playerOL[id].name) {
                lib.playerOL[id].init(result2[id]);
              }
            }
            game.zhu = zhu;
            if (!game.zhu.isInitFilter("noZhuHp")) {
      ...
      next.setContent(async (event, trigger, player) => {
        ...
        for (const current2 of game.players) {
          choices.push([
            current2,
            {
              createDialog: ["选择角色", [characterMap[current2.playerid], "character"]],
              forced: true
            }
          ]);
        }
        const result = await game.me.chooseButtonOL(choices, (current2, choiceResult) => {
          if (game.online || current2 === game.me) {
            current2.init(choiceResult.links[0], choiceResult.links[1]);
          }
        }).forResult();
    ...
    chooseCharacterBinglinOL() {
      ...
      next.setContent(async (event, trigger, player) => {
        ...
        while (!biddingFinished) {
          ...
          const result2 = await event.current.chooseControl({
            controls: [...event.controls],
            ai: () => _status.event.getParent().controls.randomGet()
          }).forResult();
          event.current._control = result2.control;
          event.current.chat(result2.control);
          if (result2.control === "三倍") {
            game.bonusNum = 3;
          ...
          if (result2.control !== "不叫地主") {
            event.controls.splice(1, event.controls.indexOf(result2.control));
    ...
    chooseCharacterOL() {
      ...
      next.setContent(async (event, trigger, player) => {
        ...
        game.broadcast((availableCharacters2) => {
          for (const name in lib.characterReplace) {
            const replacements = lib.characterReplace[name];
            for (const character of [...replacements]) {
              if (!availableCharacters2.includes(character)) {
                replacements.remove(character);
              }
            }
          }
  ...
  skill: {
    ...
    zhuSkill_jiangling0: {
      ...
      async content(event, trigger, player) {
        ...
        const result = await player.chooseTarget({
          ...
          prompt2,
          filterTarget: (_card, player2, target) => !_status.event.targets.includes(target) && lib.filter.targetEnabled2(_status.event.card, player2, target) && lib.filter.targetInRange(_status.event.card, player2, target),
          ai: (target) => {
    ...
    online_gongshoujintui: {
      ...
      filter(event, player) {
        ...
        for (const card of cards) {
          ...
          if (name === "gongshoujianbei" && (event.filterCard(
            {
              name: "wanjian",
              isCard: true,
              cards: [card]
          ...
          if (name === "jintuiziru" && (event.filterCard(
            {
              name: "nanman",
              isCard: true,
              cards: [card]
      ...
      chooseButton: {
        ...
        filter(button, player) {
          ...
          for (const card of cards) {
            if (get.name(card, player) === rawname && evt.filterCard(
              {
                name,
                isCard: true,
                cards: [card]
    ...
    kaihei: {
      ...
      async content(event, trigger, player) {
        ...
        if (hs.length <= num) {
          ...
          resultx = await player.chooseCard({
            position: "he",
            forced: true,
            selectCard: num,
    ...
    dcfeiyang: {
      ...
      async cost(event, trigger, player) {
        event.result = await player.chooseToDiscard("h", 2, get.prompt(event.skill), "弃置两张手牌，然后弃置判定区里的所有牌").set("logSkill", event.skill).set("ai", (card) => {
          if (_status.event.goon) {
            return 9 - get.value(card);
          }
          return 0;
    ...
    mbfeiyang: {
      ...
      async cost(event, trigger, player) {
        event.result = await player.chooseToDiscard("h", 2, get.prompt(event.skill), "弃置两张手牌，然后弃置判定区里的一张牌").set("logSkill", event.skill).set("ai", (card) => {
          if (_status.event.goon) {
            return 9 - get.value(card);
          }
          return 0;
    ...
    online_juzhong: {
      ...
      ruleSkill: true,
      filter(event, player) {
        return _status.mode === "online" && !event.all_excluded && event.player.isFriendOf(player) && event.player !== player && lib.skill.online_juzhong.infos[event.card.name] && player.hasCard((card) => {
          if (_status.connectMode) {
            return true;
          }
      ...
      async content(event, trigger, player) {
        const result = await player.chooseToDiscard({
          ...
          filterCard: (card, player2) => get.name(card, player2) === _status.event.getTrigger().card.name,
          ai: lib.skill.online_juzhong.infos[trigger.card.name][2]
        }).set("logSkill", ["_juzhong", trigger.player]).forResult();
        if (!result.bool) {
          return;
        }
        ...
        const targetResult = await trigger.player.chooseTarget({
          prompt: "你可选择一名角色，弃置其的一张牌",
          filterTarget: (_card, player2, target2) => target2.countDiscardableCards(player2, "he") > 0,
          ai: (target2) => {
            const player2 = _status.event.player;
      ...
      infos: {
        ...
        shan: [
          ...
          (card) => {
            if (game.zhu.countCards("he", (card2) => get.value(card2, game.zhu) >= 6)) {
              return 7 - get.value(card);
            }
            if (game.zhu.countCards("he", (card2) => get.value(card2, game.zhu) > 0)) {
    ...
    online_zhadan: {
      ...
      async content(event, trigger, player) {
        ...
        event.filterCard = (card, player2) => get.name(card) === "zhadan" && get.itemtype(card) === "card" && lib.filter.cardEnabled(card, player2, "forceEnable");
        event.send = (player2, card, source, targets, id, id2, skillState) => {
          if (skillState) {
            player2.applySkills(skillState);
          }
          if (player2 === game.me && ui.zhadan_button && !ui.zhadan_button.classList.contains("glow")) {
          ...
          const next2 = player2.chooseToUse({
            filterCard(card2, player3) {
              return get.name(card2) === "zhadan" && get.itemtype(card2) === "card" && lib.filter.cardEnabled(card2, player3, "forceEnable");
            },
            prompt,
            ...
            ai1(card2) {
              ...
              let eff = 0;
              for (const target of targets) {
                eff -= get.effect(target, evt.card, evt.player, player3);
              }
              return eff - 8;
            },
            source,
          ...
          next2.set("respondTo", [source, card]);
          if (game.online) {
            _status.event._resultid = id;
            game.resume();
          } else {
        ...
        if (!event.zhadanresult && event.list.length) {
          ...
          const choose = (current) => {
            return new Promise((resolve, reject) => {
              const settle = (result, current2) => {
                if (result?.bool && (current2 === game.me || result.id === id)) {
                  resolve([current2, result]);
                } else {
                  reject();
                }
              };
              ...
              if (response?.forResult) {
                response.forResult().then((result) => game.me.unwait(result)).catch(reject);
              } else {
  ...
  card: {
    baiyidujiang: {
      ...
      async content(event, trigger, player) {
        ...
        for (const current of game.filterPlayer()) {
          const num3 = current.countCards("h");
          if (num3 > num1) {
            num1 = num3;
          }
          if (num3 < num2) {
      ...
      ai: {
        ...
        result: {
          target(player, target, card, isLink) {
            ...
            for (const current of game.filterPlayer()) {
              const num3 = current.countCards("h", filterCard);
              if (num3 > num1) {
                num1 = num3;
              }
              if (num3 < num2) {
      ...
      ai: {
        ...
        result: {
          target(player, target, card, isLink) {
            ...
            let val = 0;
            for (const card2 of es) {
              val += get.value(card2, target);
            }
