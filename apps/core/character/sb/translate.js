...
const translates = {
	...
	sb_zhanghe: "谋张郃",
	sbqiaobian: "巧变",
	sbqiaobian_info: "准备阶段，你可以弃置任意区域里的各一张牌，跳过等量个阶段（判定阶段和结束阶段除外），并于跳过对应阶段后执行对应效果。若以此法跳过：摸牌阶段，你可以获得至多两名其他角色各一张手牌；出牌阶段，你可以移动场上的一张牌，然后若你的牌数未增加，你可以令牌数因此增加的角色摸两张牌；弃牌阶段，你摸一张牌，然后结束阶段，若你的手牌数不小于场上角色数，你执行一个额外的出牌阶段。",
	sbliaoshi: "料势",
	//sbliaoshi_info: `每回合限一次，你可以将一张因${get.poptip("sbqiaobian")}获得的牌当任意基本牌使用或打出。`,
	sbliaoshi_info: `每回合限一次，你可以将一张因${get.poptip("sbqiaobian")}进入你装备区或手牌区的牌当【杀】或【闪】使用或打出。`,
	old_sbqiaobian: "巧变",
