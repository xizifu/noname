...
const skills = {
  ...
  stdzhuikong: {
    ...
    filter(event, player) {
      ...
      return _status.connectMode && player.hasCards("h") || player.hasCards("h", (card2) => get.name(card2) == "sha");
    },
    async cost(event, trigger, player) {
      event.result = await player.chooseCard(get.prompt(event.skill, trigger.player), "使用一张【杀】与其拼点", (card2) => {
        return get.name(card2) == "sha";
      }).set("ai", (card2) => {
  ...
  stdchunlao: {
    ...
    filter(event, player) {
      return (event.cards || []).length >= 2 && game.hasPlayer((target2) => target2 != player);
    },
    async cost(event, trigger, player) {
      const cards2 = trigger.cards;
      event.result = await player.chooseTarget(get.prompt(event.skill), "用" + get.translation(cards2) + "交换一名其他角色的手牌", (card2, player2, target2) => target2 != player2).set("ai", (target2) => {
        const { cards: cards3, player: player2 } = get.event();
        const att = get.attitude(player2, target2);
        return (cards3.length - target2.countCards("h") + 0.1) * att;
