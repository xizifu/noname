...
const skills = {
  ...
  natulie: {
    ...
    filter(event, player2) {
      ...
      Math.min(5, player2.countHistory("useSkill", (evt) => evt.skill == "natulie") + 1);
      return true;
    },
    prompt2(event, player2) {
      const num = Math.min(5, player2.countHistory("useSkill", (evt) => evt.skill == "natulie") + 1);
      return `将手牌调整至${num}，若你因此弃置了牌，你可对一名角色造成一点伤害`;
    },
    check(event, player2) {
      const num = Math.min(5, player2.countHistory("useSkill", (evt) => evt.skill == "natulie") + 1);
      return player2.countCards("h") < num || player2.countCards("h") - num <= 2 && game.hasPlayer((current) => get.damageEffect(current, player2, player2));
    },
    async content(event, trigger, player2) {
      const num = Math.min(
        5,
        player2.countHistory("useSkill", (evt) => evt.skill == "natulie")
      );
      if (player2.countCards("h") < num) {
        await player2.drawTo(num);
      } else if (player2.countCards("h") > num) {
        const { cards: cards2 } = await player2.chooseToDiscard({ forced: true, position: "h", selectCard: player2.countCards("h") - num }).forResult();
        if (cards2?.length) {
  ...
  natuheng: {
    ...
    mod: {
      cardUsable(card2) {
        if (card2?.storage?.natuheng) {
          return Infinity;
        }
      }
    },
    filter(event, player2) {
      if (player2.hasSkill("natuheng_round")) {
        return false;
      }
      return !player2.hasCards("h", (card2) => !get.is.damageCard(card2)) && event.filterCard(get.autoViewAs({ name: "shan", isCard: true }, "unsure"), player2, event) || !player2.hasCards("h", (card2) => get.is.damageCard(card2)) && event.filterCard(get.autoViewAs({ name: "sha", isCard: true, storage: { natuheng: true } }, "unsure"), player2, event);
    },
    chooseButton: {
      dialog(event, player2) {
        const list = get.inpileVCardList((info) => {
          ...
          if (info[2] == "sha" && player2.hasCards("h", (card2) => get.is.damageCard(card2))) {
            return false;
          }
          if (info[2] == "shan" && player2.hasCards("h", (card2) => !get.is.damageCard(card2))) {
            return false;
          }
      ...
      backup(links, player2) {
        return {
          ...
          async precontent(event, trigger, player3) {
            player3.logSkill("natuheng");
            event.getParent().addCount = false;
            player3.addTempSkill("natuheng_round", "roundStart");
          }
        };
      },
    ...
    hiddenCard(player2, name) {
      if (!lib.inpile.includes(name) || player2.hasSkill("natuheng_round")) {
        return false;
      }
      return name == "shan" && !player2.hasCards("h", (card2) => !get.is.damageCard(card2)) || name == "sha" && !player2.hasCards("h", (card2) => get.is.damageCard(card2));
    },
    subSkill: {
    ...
    ai: {
      ...
      skillTagFilter(player2, tag) {
        ...
        return get.info("natuheng").hiddenCard(player2, name);
      },
      order: 1,
      result: {
        player: 1
  ...
  natuxing: {
    ...
    isXing(card2) {
      const info = lib.card[card2.name];
    ...
    filter(event, player2) {
      if (event.name == "useCard") {
        return !player2.hasCards("h", (card2) => card2.hasGaintag("natuxing"));
      }
    ...
    async content(event, trigger, player2) {
      if (trigger.name == "useCard") {
        ...
        for (let i = 0; i < ui.cardPile.childNodes.length; i++) {
          ...
          if (get.info(event.name).isXing(card2) && !cards2.map((card3) => get.name(card3)).includes(card2.name)) {
            cards2.push(card2);
          }
        }
        if (cards2.length) {
          await player2.gain({ cards: cards2, animate: "gain2", gaintag: [event.name] });
        }
      }
    },
    mod: {
      ignoredHandcard(card2, player2) {
        if (card2.hasGaintag("natuxing")) {
          return true;
        }
      },
      ...
      cardEnabled(card2, player2) {
        ...
        if (cards2?.some((card3) => card3.hasGaintag("natuxing"))) {
          return false;
        }
      },
      cardUsable(card2, player2) {
        if (!player2.hasCards("h", (card3) => card3.hasGaintag("natuxing"))) {
          return Infinity;
        }
      },
    ...
    subSkill: {
      clear: {
        ...
        usable(skill, player2) {
          return player2.countHistory("useCard", (evt) => get.is.damageCard(evt.card));
        },
        filter(event, player2) {
          return player2.hasCards("h", (card2) => card2.hasGaintag("natuxing"));
        },
        async content(event, trigger, player2) {
          const card2 = player2.getCards("h", (card3) => card3.hasGaintag("natuxing")).randomGet();
          player2.removeGaintag("natuxing", card2);
        }
      }
    }
  },
  natufu: {
    ...
    filter(event, player2) {
      ...
      const bool1 = !player2.hasCards("h", (card2) => card2.hasGaintag("natuxing") && card2.name == event.card.name) && game.hasPlayer((current) => current.isMinHp(true) && current.isDamaged());
    ...
    prompt2(event, player2) {
      let str = "", num = 1;
      const bool11 = !player2.hasCards("h", (card2) => card2.hasGaintag("natuxing") && card2.name == event.card.name);
    ...
    check(event, player2) {
      ...
      if (bool1) {
        ...
        effect += get.recoverEffect(target, player2, player2) * num;
      }
      if (bool2) {
        const nums = game.filterPlayer().map((current) => get.damageEffect(current, player2, player2, "fire") * 2);
        nums.sort((a, b) => b - a);
        effect += nums[0];
      }
      return effect;
    ...
    async content(event, trigger, player2) {
      let num = 1;
      const bool11 = !player2.hasCards("h", (card2) => card2.hasGaintag("natuxing") && card2.name == trigger.card.name);
      const bool1 = bool11 && game.hasPlayer((current) => current.isMinHp(true) && current.isDamaged());
      const bool2 = !game.hasPlayer((current) => {
        if (current == player2) {
          return false;
      ...
      if (bool2) {
        const result = await player2.chooseTarget({
          ...
          ai(target) {
            ...
            return get.event().num * get.damageEffect(target, player3, player3, "fire");
          }
  ...
  natuyi: {
    ...
    usable: 1,
    filter(event, player2) {
      return game.hasPlayer((current) => get.info("natuyi").filterTarget(null, player2, current));
    },
    ...
    async content(event, trigger, player2) {
      ...
      await game.doAsyncInOrder(targets, async (target) => {
        if (target.hasCards("he")) {
          const { cards: cards2 } = target == player2 ? await target.chooseCard({
            forced: true,
            position: "he",
            prompt: "义：请选择一张牌",
            ai(card3) {
          ...
          }).forResult() : await target.chooseToGive({
            target: player2,
            forced: true,
            position: "he",
            prompt: `${get.translation(player2)}对你发动了【义】：请交给其一张牌`,
            ai(card3) {
          ...
          if (cards2?.length) {
            ...
            if (target == player2 && get.position(cards2[0]) == "e") {
              await player2.gain({ cards: cards2, animate: "gain2" });
            }
          }
        }
      });
      cardx = player2.getCards("he").filter((card3) => cardx.includes(card3));
      if (!cardx.length) {
        return;
      }
      game.broadcastAll((cardx2) => {
        cardx2.forEach((card3) => card3.addGaintag("natuyi"));
      }, cardx);
      let result;
      const colors = cardx.map((card3) => get.color(card3)).unique();
      const list = [];
      let card2 = get.autoViewAs({ name: "sha", storage: { natuyi: true } }, cardx);
      if (targets.some((target) => player2.canUse(card2, target, false, false))) {
        list.push("使用杀");
      }
      ...
      result = list.length > 1 ? await player2.chooseControl({
        controls: list,
        prompt: "义：请选择一项",
        ai() {
      ...
      player2.logSkill(event.name, null, null, null, [2]);
      if (result?.control == "使用杀") {
        const list2 = get.inpileVCardList((info) => {
          if (info[2] != "sha") {
            return false;
          }
          const card3 = get.autoViewAs({ name: info[2], nature: info[3], storage: { natuyi: true } }, cardx);
          return targets.some((target) => player2.canUse(card3, target, false, false));
        });
        if (!list2.length) {
          return;
        }
        result = await player2.chooseButton({
          ...
          forced: true,
          ai(button) {
            const card3 = get.autoViewAs({ name: button.link[2], nature: button.link[3], storage: { natuyi: true } }, get.event().cardx);
            return player2.getUseValue(card3);
          }
        }).set("cardx", cardx).forResult();
        if (!result?.bool || !result.links?.length) {
          return;
        }
        card2 = get.autoViewAs({ name: result.links[0][2], nature: result.links[0][3], storage: { natuyi: true } }, cardx);
        const next = player2.chooseUseTarget(card2, true, false, "nodistance", cardx, `请选择【杀】的目标（${get.translation(cardx)}）`).set("filterTarget", (card3, player3, target) => {
          return player3.canUse(card3, target, false, false) && get.event().targets.includes(target);
        ...
        player2.when({ global: ["useCardAfter", "useCardToExcluded", "eventNeutralized", "shaMiss", "useCardToEnd"] }).filter((evt) => evt.card?.storage?.natuyi).step(async (event2, trigger2, player3) => {
      ...
      if (result?.control == "分配牌") {
        if (!game.hasPlayer((current) => !targets.includes(current) && player2 != current)) {
          player2.addSkill(event.name + "_effect");
          return;
        }
        ...
        const given_map = {};
        while (cardx.some((card3) => !card3.hasGaintag("qiaoli_given"))) {
          result = await player2.chooseCardTarget({
            filterCard(card3) {
              return get.event().cardx.includes(card3) && !card3.hasGaintag("qiaoli_given");
            },
            filterTarget(card3, player3, target) {
          ...
          if (result.bool && result.cards?.length && result.targets?.length) {
            ...
            player2.addGaintag(cards2, "qiaoli_given");
    ...
    subSkill: {
      effect: {
        ...
        filter(event, player2) {
          if (!event.cards?.length || !event.cards.some((card2) => get.color(card2) == "black") || event.target == player2) {
            return false;
          }
        ...
        async content(event, trigger, player2) {
          ...
          if (!player2.hasCards("h", (card2) => card2.hasGaintag("natuyi"))) {
      ...
      nouse: {
        ...
        mod: {
          cardEnabled(card2, player2) {
            ...
            if (cards2.length) {
              if (cards2.map((card3) => get.color(card3)).unique().containsSome(...player2.getStorage("natuyi_nouse"))) {
                return false;
              }
            }
          }
  ...
  olweidai: {
    ...
    filter(event, player2) {
      return event.player.group == "wei" && event.targets?.includes(player2);
    },
    async content(event, trigger, player2) {
      await player2.gainMaxHp();
      if (player2.isMaxMaxHp()) {
  ...
  olliangzi: {
    ...
    mod: {
      cardEnabled(card2, player2) {
        if (get.name(card2) === "jiu") {
          return;
        }
        const hs = player2.getCards("he", (card3) => get.type(card3) == "equip");
        if (get.type(card2) == "equip" && "cards" in card2 && Array.isArray(card2.cards) && card2.cards.containsSome(...hs)) {
          return false;
        }
      },
    ...
    viewAsFilter(player2) {
      return player2.countCards("he", (card2) => get.type(card2) == "equip") > 0;
    },
    viewAs: {
      name: "jiu"
    },
    ...
    subSkill: {
      use: {
        ...
        filter(event, player2) {
          ...
          const index = player2.getHistory("useCard", (evt) => get.name(evt.card) == "jiu").indexOf(event);
  ...
  dczuandai: {
    ...
    check(event, player2) {
      ...
      return hs.length < 3 && hs.every((card2) => !player2.hasUseTarget(card2));
    },
    async content(event, trigger, player2) {
  ...
  dcsitian: {
    ...
    weathers: {
      ...
      暴雨: {
        ...
        async content(event, trigger, player2) {
          const result = await player2.chooseTarget("请选择【暴雨】的目标", "令目标角色弃置所有手牌。若其没有手牌，则其改为失去1点体力。").set("ai", (current) => {
...
const translates = {
  ...
  natuyi: "义",
  natuyi_info: "出牌阶段限一次，你可以令至多两名角色交给你一张牌，然后你选择一项：1.将这些牌当做无距离次数限制的【杀】对其中一名角色使用。此【杀】结算中，其不能使用与此【杀】转化前的牌相同颜色的手牌；2.将这些牌分配给未成为目标的角色，其使用因此获得的黑色牌指定其他角色为目标后，目标本回合非锁定技失效。",
  natuxing: "兴",
  ...
  natufu: "抚",
  natufu_info: `你使用锦囊牌后，可发动此技能。若此牌：1.与你手牌中的“兴”牌名均不同，你令体力值唯一最小的角色回复1点体力；2.本局游戏仅你使用过次牌名的牌，你对一名角色造成1点火属性伤害。${get.poptip("rule_chengshi")}：本次执行效果中所有数字+1。`,
  natulie: "烈",
  natulie_info: "你使用基本牌后，你可将手牌数调整至X张（X为本回合本技能发动次数+1且至多为5），若你因此弃置了牌，您可对一名角色造成1点伤害。",
  natuheng: "恒",
  natuheng_info: "每轮限一次，若你的手牌中没有：1.非伤害牌，你可视为使用或打出一张【闪】；2.伤害牌，你可视为使用或打出一张无次数限制的【杀】。",
  ol_le_caopi: "OL乐曹丕",
...
const characterSort = {
  ...
  collab_oldoudizhu: ["ol_le_zhugeliang", "ol_le_caohong", "ol_le_liushan", "bigsb_dengai", "wild_liru", "strong_caochong", "taipingsangong", "wuhujiang", "ol_jsrg_caocao", "ol_jsrg_lvbu", "ol_nianshou", "hanshiwuhu", "ol_xiahouen", "ol_le_caopi"],
