...
const skills = {
  ...
  sxrmjiaozong: {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseTarget({
        ...
        filterTarget(card, player2, target) {
          return target.hasCards("e", (card2) => game.hasPlayer((current) => current !== target && current.canEquip(card2)));
        },
        ai(target) {
          const player2 = get.player();
          let val = Math.sign(get.attitude(player2, target));
          if (val > 0) {
            if (target.hasSkill("xiaoji")) {
              val += 4;
            }
            if (game.hasPlayer((current) => get.attitude(player2, current) < 0 && target.hasCards("e", (card) => get.color(card) === "red" && current.canEquip(card) && player2.inRange(current) && _status.currentPhase === player2))) {
              val += 5;
            }
          } else {
            if (game.hasPlayer((current) => get.attitude(player2, current) > 0 && target.hasCards("e", (card) => current.canEquip(card)) && current !== player2)) {
              val = -val + 3;
            }
          }
          return val;
    ...
    async content(event, trigger, player) {
      ...
      let result = await player.choosePlayerCard({
        ...
        ai(button) {
          ...
          const from = event2.from;
          const player2 = get.player();
          let val = get.value(button.link, from);
          if (get.attitude(player2, from) < 0) {
            return 5 - val;
          }
          return val;
        }
      }).set("from", target).forResult();
      ...
      result = await player.chooseTarget({
        ...
        filterTarget(card2, player2, target3) {
          ...
          return target3.canEquip(card0) && target3 !== target3;
        },
        forced: true
      }).set("ai", (target3) => {
        const player2 = get.player();
        const { card0 } = get.event();
        let val = Math.sign(get.attitude(player2, target3));
        if (val < 0) {
          if (player2.inRange(target3) && player2 == _status.currentPhase) {
            val = -val + get.color(card0) == "red" ? 3 : 2;
          } else if (target3 == _status.currentPhase) {
            val = -val + 4;
          } else {
            val = -val;
          }
        } else {
          if (target3 == player2 && player2 == _status.currentPhase) {
            val -= 5;
          }
          if (target3.hasSkillTag("nodamage")) {
            val += 5;
          } else {
            val += 1;
          }
  ...
  sxrmfusui: {
    ...
    filterTarget(card, player, target) {
      ...
      return target !== player && target.sex === "male" && skills2.length > 0;
    },
    async content(event, trigger, player) {
      const target = event.targets[0];
      player.awakenSkill("sxrmfusui");
      const skills2 = target.getSkills(null, false, false).filter((skill2) => {
        let list = get.skillCategoriesOf(skill2, player);
        return !list.length;
      });
      if (!skills2.length) {
        return;
      }
      const { control: skill } = await player.chooseControl({
  ...
  sxrmxiezhong: {
    ...
    async content(event, trigger, player) {
      ...
      for (const target2 of targets) {
        ...
        if (!lib.skill.sxrmxiezhong.canUseSha(target2)) {
          ...
          list.push([target2, result2.index]);
        }
      }
      if (!list.length) {
        return;
      ...
      for (const [target2, index] of list) {
        if (!target2.isIn()) {
          continue;
        }
        if (index == 0) {
    ...
    canUseSha(player) {
      let cards2 = player.getCards("he");
      if (cards2.length < 2) {
        return false;
      }
      return cards2.some((card1) => {
  ...
  sxrmqishi: {
    ...
    async cost(event, trigger, player) {
      ...
      const cards2 = lib.skill.sxrmqishi.getZhongyangCards(player);
      const result = await player.chooseButton({
        createDialog: ["获得本回合其他角色进入弃牌堆的至多五张牌", cards2],
        selectButton: [1, 5],
        forced,
        ai(button) {
    ...
    async content(event, trigger, player) {
      ...
      const cards2 = event.cost_data.links;
      player.gain({
        cards: cards2,
        animate: "gain2"
      });
  ...
  sxrmwanli: {
    ...
    async content(event, trigger, player) {
      const target = event.targets[0];
      const cards2 = await player.chooseCard({
        position: "he",
      ...
      eff: {
        ...
        async content(event, trigger, player) {
          ...
          for (let i in items) {
            ...
            let loseSkill, cards2;
            if (target.countCards("he") <= num) {
              cards2 = target.getCards("he");
              if (target.countCards("he") < num) loseSkill = true;
            } else {
              cards2 = await target.chooseCard({
            ...
            target.line(player);
            if (cards2?.length) await target.give(cards2, player);
  ...
  sxrmlishui: {
    ...
    async content(event, trigger, player) {
      ...
      player.getStorage("sxrmwanli_eff").forEach((item) => {
        const { target: target2, huanqian } = item;
        if (!huanqian && target2.isIn() && player.canCompare(target2)) targets.push(target2);
      });
      if (!targets.length) {
        return;
      }
      const result = await player.chooseTarget({
        prompt: "选择一名“万利”角色拼点",
        filterTarget: (card, player2, target2) => {
          return targets.includes(target2);
        },
        forced: true
      }).forResult();
      const target = result.targets[0];
  ...
  sxrmlanjiao: {
    ...
    async content(event, trigger, player) {
      ...
      while (1) {
        ...
        cards2 = await player.choosePlayerCard({
          target,
          position: "h",
          selectButton: 2
        }).forResult("cards");
        await player.showCards(cards2, `${get.translation(player)}对${get.translation(target)}发动了【揽娇】`);
        let rechoose = await target.chooseBool({
          prompt: `是否失去一点体力并摸一张牌，令${get.translation(player)}重新选择？`,
          ai: () => {
            const player2 = get.player(), cards3 = get.event().cards;
            return player2.hp > 3 && !cards3.some((i) => get.color(i) == "red");
          }
        }).set("cards", cards2).forResult("bool");
        if (rechoose) {
          await target.draw();
          await target.loseHp();
        } else {
      ...
      used: {
        ...
        intro: {
          content(storage, player) {
            ...
            let str1 = targets.map((i) => get.translation(i)).join("、"), str2 = suits.map((i) => get.translation(i)).join("、");
  ...
  sxrmbingqu: {
    ...
    getVcards(player) {
      ...
      let vcard = list.filter((name) => {
        return lib.filter.cardEnabled({ name }, player);
      }).map((namex) => {
    ...
    async content(event, trigger, player) {
      ...
      for (let playerx of [player, target]) {
        ...
        const { cards: cards2 } = await playerx.chooseCard({
          position: "h",
          selectCard: Math.ceil(playerx.countCards("h") / 2),
          forced: true,
          prompt: `将一半手牌当作${get.translation(name)}使用`,
          filterCard: (card, player2) => {
            return game.checkMod(card, player2, "unchanged", "cardEnabled2", player2) !== false;
          },
          ai: (card) => {
            return 10 - get.value(card);
          }
        }).forResult();
        const cardx = get.autoViewAs({ name }, cards2);
  ...
  sxrmfanxin: {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseTarget({
        ...
        filterTarget: (card, player2, target) => {
          return target != player2 && !player2.storage["sxrmfanxin"]?.includes(target);
        },
        ai: (target) => {
          const player2 = get.player();
  ...
  sxrmfubei: {
    ...
    async content(event, trigger, player) {
      ...
      let { cards: cards2 } = await player.chooseCard({
        prompt: "选择置入牌堆的两张牌",
        forced: true,
        selectCard: 2,
        position: "he",
    ...
    subSkill: {
      ...
      clear: {
        ...
        async content(event, trigger, player) {
          let cards2 = player.getStorage("sxrmfubei_damage");
  ...
  sxrmdancui: {
    ...
    async cost(event, trigger, player) {
      const qiding = Boolean(player.storage["sxrmdancui"]), target = trigger.player, description = lib.dynamicTranslate["sxrmdancui"](player, "sxrmdancui");
      if (player.countDiscardableCards(player, "he") < 2) {
        ...
        const result = await player.chooseCard({
          ...
          selectCard: 2,
          filterCard: (card, player2) => {
            return player2.getDiscardableCards(player2, "he").includes(card);
          },
          ai: (card) => {
            const player2 = get.player(), target2 = get.event().target;
    ...
    async content(event, trigger, player) {
      ...
      if (cards2?.length) await player.discard(cards2);
      trigger.num++;
    },
    mark: true,
    intro: {
  ...
  sxrmzhaduo: {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseTarget({
        ...
        selectTarget: 2,
        filterTarget: (card, player2, target) => {
          return player2 != target && target.countGainableCards(player2, "he");
        },
        ai: (target) => {
          const player2 = get.player();
          return -get.attitude(player2, target);
    ...
    async cost(event, trigger, player) {
      if (event.triggername != "useCardToPlayered") {
        ...
        event.result = await target.chooseBool({
          ...
          ai: () => {
            const target2 = get.player(), { playerx } = get.event;
            if (get.attitude(target2, playerx) >= 0) return false;
            if (!playerx.isTurnedOver()) return false;
            switch (target2.countCards("h")) {
              case 1:
                return true;
              case 2:
              case 3:
                return target2.countCards("h", (card) => {
    ...
    async content(event, trigger, player) {
      ...
      player.when({ player: "phaseBegin" }).then(() => {
        ...
        for (let i of player.storage["sxrmyinfeng"]) {
          ...
          player.line(i);
          const num2 = i.storage["sxrmyinfeng_debuff"];
          i.gainMaxHp(num2);
...
const dynamicTranslates = {
  ...
  sxrmbiyi(player, skill) {
    ...
    let yang = sk ? `【${get.translation(sk)}】` : "无技能", yin = "【枭姬】";
    if (bool) {
