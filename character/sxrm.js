  ...
  sxrmchanyu: {
    ...
    async content(event, trigger, player) {
      ...
      if (result?.winner) {
        ...
        const list2 = lib.inpile.map((name) => get.type2(name)).toUniqued().filter((type) => [winner, loser].some((current) => current.hasCards("h", (card) => get.type2(card) == type)));
        const result2 = await winner.chooseControl(list1.concat(list2), "cancel2").set("dialog", dialog).set("ai", () => get.event().resultx).set(
          "resultx",
          (() => {
